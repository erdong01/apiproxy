...
<template>
  <div>
   ...
   <template #default="scope">{{ formatDate(scope.row.deletedAt) }}</template>
            ...
            <el-table-column align="left" label="供应商" prop="provider" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updatePqAiModelFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getPqAiModelList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createPqAiModel(formData.value)
                  break
                case 'update':
                  res = await updatePqAiModel(formData.value)
                  break
                default:
                  res = await createPqAiModel(formData.value)
                  break
